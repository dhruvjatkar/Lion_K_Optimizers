...
from airbench.lib_airbench96 import hyp, make_net96
...
def _fidelity_epochs_and_tta(base_epochs: float, base_tta: int, fidelity: str) -> tuple[float, int]:
    if fidelity == "F1":
        return max(2.0, min(base_epochs, 6.0)), 0
    if fidelity == "F2":
...
def _build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Airbench96 transfer runner with MuonK matrix optimizer")
    p.add_argument("--runs", type=int, default=25)
    p.add_argument("--seed", type=int, default=0)
    p.add_argument("--fidelity", choices=["F1", "F2", "F3"], default="F3")
    p.add_argument("--target-acc", type=float, default=0.96)
